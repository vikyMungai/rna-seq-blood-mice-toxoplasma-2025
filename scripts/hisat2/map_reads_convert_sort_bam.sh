...
#SBATCH --output=/data/users/vmungai/rna_seq_projects/rna-seq-blood-mice-toxoplasma-2025/logs/outputs/hisat2/map_reads_convert_sort_bam/output_map_%A_%a.out
#SBATCH --error=/data/users/vmungai/rna_seq_projects/rna-seq-blood-mice-toxoplasma-2025/logs/errors/hisat2/map_reads_convert_sort_bam/error_map_%A_%a.err
...
BAM_FILES_DIR="$WORKDIR/results/hisat2/map_reads_convert_sort_bam"
...
apptainer exec --bind /data/ \
     $CONTAINER_HISAT_SAMTOOLS hisat2 -x "$INDEX_BASENAME" -1 $READ1 -2 $READ2 -S $SAM_FILE -p $SLURM_CPUS_PER_TASK
